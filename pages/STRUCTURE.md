...
- Хедер главной теперь ведёт на страницы, а не на якоря: он одинаковый на
  всех восьми файлах.
- Бургер-меню — те же 8 пунктов везде.
- Футер разбит по разделам Betting / Casino / About со ссылками на страницы.
- Все 7 карточек секции 4 стали ссылками на соответствующие страницы.
- FAQ на главной сокращён с 9 вопросов до 5 — остались те, что не дублируются
  на тематических страницах (что такое Wrbet, платёжные методы, налоги,
  проверка лицензии, куда обращаться за помощью). Полный список — на
  `faq.html`, ссылка стоит под блоком.
- Вопросы про коэффициенты, live-ставки, RTP и crash-раунд убраны с главной:
  они целиком есть на своих страницах и в FAQ.

## Автономные сборки

`tools/build-single-file.py` собирает каждую страницу в `standalone/<имя>.html`
со вшитыми CSS, JS и шрифтами — открывается двойным щелчком, папка `assets`
не нужна, ссылки между страницами внутри папки работают. Главная
дополнительно кладётся в корень как `wrbet-kenya-landing.html`.
